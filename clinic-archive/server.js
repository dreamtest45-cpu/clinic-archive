...
app.post('/api/patients', async (req, res) => {
  try {
    const b = req.body || {};
    const name = str(b.name), phone = str(b.phone);
    ...
    const { rows } = await pool.query(
      ...
      [id, name, phone, str(b.phone2), str(b.email), str(b.dob), str(b.gender), str(b.national_id),
...
app.patch('/api/patients/:id', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      ...
      [name, phone, str(b.phone2), str(b.email), str(b.dob), str(b.gender), str(b.national_id),
...
app.post('/api/patients/:id/visits', async (req, res) => {
  ...
  try {
    ...
    const id = crypto.randomUUID();
    const { rows } = await pool.query(
      `INSERT INTO visits (id, patient_id, date, time, reason, diagnosis, treatment, doctor, notes)
