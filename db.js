...
async function migrate() {
  ...
  // Existing deployments already have the `visits` table from before insurance
  // tracking existed — CREATE TABLE IF NOT EXISTS above won't add columns to an
  // already-created table, so add them explicitly (no-op on a fresh database).
  await pool.query(`
    ALTER TABLE visits ADD COLUMN IF NOT EXISTS has_insurance BOOLEAN NOT NULL DEFAULT false;
    ALTER TABLE visits ADD COLUMN IF NOT EXISTS insurance_company TEXT;
  `);
}
